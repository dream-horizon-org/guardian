...
public class CredentialsQuery {
  public static final String GET_CREDENTIAL_BY_DEVICE_ID =
      """
        SELECT id, tenant_id, client_id, user_id, device_id, platform, credential_id, public_key,
        binding_type, alg, sign_count, aaguid, is_active, first_use_complete
        FROM credentials
  ...
  public static final String INSERT_CREDENTIAL =
      """
        INSERT INTO credentials (tenant_id, client_id, user_id, device_id, platform, credential_id,
        public_key, binding_type, alg, sign_count, aaguid, revoked_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)
      """;
}
