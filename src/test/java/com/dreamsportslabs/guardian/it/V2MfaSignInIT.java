...
public class V2MfaSignInIT {
  ...
  public void returnValidTokenWhenPasswordFactorIsUsedAfterPasswordlessSignin() {
    ...
    StubMapping getUserStub =
        stubGetUserForPasswordless(userId, phoneNumber, null, false, true, false);
    ...
    validateAccessTokenClaimsForMfa(
        accessToken, TEST_SCOPE, TENANT_ID, firstPartyClientId, expiresIn, List.of("otp", "pwd"));
  ...
  public void returnValidTokenWhenPinFactorIsUsedAfterPasswordlessSignin() {
    ...
    String userId = RandomStringUtils.randomAlphanumeric(10);
    StubMapping getUserStub = stubGetUserForPasswordless(userId, null, email, false, false, true);
    ...
    int expiresIn = response.jsonPath().getInt("expires_in");
    validateAccessTokenClaimsForMfa(
        accessToken, TEST_SCOPE, TENANT_ID, firstPartyClientId, expiresIn, List.of("otp", "pin"));
  ...
  public void returnValidTokenWhenUsingEmailIdentifierWithPasswordFactor() {
    ...
    String email = generateRandomEmail();
    String userId = RandomStringUtils.randomAlphanumeric(10);
    StubMapping getUserStub = stubGetUserForPasswordless(userId, null, email, false, true, false);
  ...
  public void returnValidTokenWhenUsingPhoneNumberIdentifierWithPinFactor() {
    ...
    String userId = RandomStringUtils.randomAlphanumeric(10);
    StubMapping getUserStub =
        stubGetUserForPasswordless(userId, phoneNumber, null, false, false, true);
  ...
  public void returnErrorWhenFactorIsMissing() {
    ...
    String phoneNumber = generateRandomPhoneNumber();
    String userId = RandomStringUtils.randomAlphanumeric(10);
    StubMapping getUserStub = stubGetUserForPasswordless(userId, phoneNumber, null, false);
  ...
  private StubMapping stubGetUserForPasswordless(
      String userId, String phoneNumber, String email, boolean isNewUser) {
    return stubGetUserForPasswordless(userId, phoneNumber, email, isNewUser, false, false);
  }
  ...
  private StubMapping stubAuthenticateUserSuccess(
      String userId, String username, String email, String phoneNumber, String credentialType) {
  ...
  private void validateAccessTokenClaimsForMfa(
      String accessToken,
      String scope,
      String tenantId,
      String clientId,
    ...
    for (String authMethod : authMethods) {
      assertThat(amr.contains(authMethod), equalTo(true));
    }
    assertThat(amr.size(), equalTo(authMethods.size()));
  }
